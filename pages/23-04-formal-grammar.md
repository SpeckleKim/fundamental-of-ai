...
CFG는 촘스키 계층(Chomsky hierarchy)에서 Type-2 자리에 앉는다. 1956년 촘스키가 형식문법을 규칙이 얼마나 자유로운가에 따라 네 층으로 가지런히 나눈 것이 이 계층이다. 위로 올라갈수록 규칙이 풀려나 표현력이 커지고, 그만큼 그 언어를 알아보는 데 더 강력한 기계가 필요해진다. 한눈에 보면 이렇게 정리된다.

| 유형 | 문법 | 규칙의 모양 | 인식하는 기계 | 쓰임새 |
|---|---|---|---|---|
| Type-3 | 정규문법 | 한쪽으로만 자라나는 가장 빡빡한 규칙 | 유한상태기계 | 검색 패턴(정규식), 어휘 구조 |
| Type-2 | 문맥자유문법(CFG) | 왼쪽이 변수 하나 | 푸시다운 자동기계 | 거의 모든 프로그래밍 언어 문법 |
| Type-1 | 문맥의존문법 | 앞뒤 문맥에 따라 치환 | 선형한정 오토마타 | 문맥을 따지는 자연어 현상 |
| Type-0 | 무제한문법 | 아무 제약 없는 규칙 | 튜링기계 | 계산 가능한 모든 언어 |

표를 아래에서 위로 거슬러 읽으면 곧 표현력의 사다리다. 가장 빡빡한 Type-3(유한상태기계)에서 출발해, 가장 무엇이든 할 수 있는 Type-0(튜링기계)에 이른다. CFG는 그 한가운데에 자리 잡은, **거의 모든 프로그래밍 언어 문법의 이론적 토대**다. 맨 위 칸의 튜링기계가 낯익을 것이다 — 그렇다, 이 계산능력 이야기는 자동기계·계산이론(9장)과 곧장 맞닿아 있다(6장의 형식체계와도 한 뿌리에서 갈라져 나왔다).
...
둘째 규칙이 정의 한가운데서 자기 자신(`<name-part>`)을 다시 불러내는 점에 주목하자. 이 **재귀(recursion)** 덕분에 손에 꼽을 만큼 적은 규칙으로 무한히 많은 문장을 다룰 수 있게 된다. 일상에서도 우리는 이 힘을 쓴다 — "사과**와** 배", "사과와 배**와** 귤", "사과와 배와 귤**과** 포도…"처럼 '와/과'로 끝없이 잇는 문장을 떠올려 보라. "명사구 = 명사구 **그리고** 명사구"라는 단 한 줄의 재귀 규칙이, 무한히 긴 목록을 모두 품는다. 형식문법의 힘은 바로 이 작은 되먹임 고리에 있다.

## 심화 · 격문법과 몬태규문법

> 💡 **심화 — 건너뛰어도 좋다.** 구문의 뼈대(앞 절)를 잡았다면 충분하다. 여기서는 그 너머 '의미'를 겨눈 더 깊은 문법 이론을 맛본다.
