...
여기서 '약간 높다'와 '세게'는 흑백으로 딱 떨어지지 않는다. 저마다 소속도를 거느린 퍼지 개념이다. 여러 규칙이 동시에 *정도껏* 발동하고, 그 결과들을 한데 모아 다시 또렷한 숫자로 풀어내면(비퍼지화) 비로소 최종 출력이 나온다.

한 걸음씩 따라가 보자. 지금 방이 28도라 하자. 이 온도는 '적당하다'에 0.3, '약간 높다'에 0.7만큼 걸쳐 있다(어느 한쪽이 아니라 *둘 다 정도껏*). 그러면 두 규칙이 함께, 그러나 세기를 달리해 발동한다 — "적당하면 약하게"가 0.3의 힘으로, "약간 높으면 세게"가 0.7의 힘으로. 마지막으로 이 둘을 *세기에 비례해 가중평균*하면 하나의 숫자로 모인다. 그 결과가 이를테면 **에어컨 세기 73%** 다. 켜짐/꺼짐의 딱딱한 스위치가 아니라, 사람이 손잡이를 슬며시 돌리듯 부드럽게 조절되는 것이다.
