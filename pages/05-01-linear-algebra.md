...
특히 내적(dot product)은 두 벡터가 얼마나 같은 방향을 바라보는지를 단 하나의 숫자로 일러 준다. 계산은 의외로 쉽다 — *짝끼리 곱해 모두 더하면* 끝이다. 예컨대 (1, 2)와 (3, 1)의 내적은 1×3 + 2×1 = **5**다. 핵심은 그 값이 방향이 맞을수록 커진다는 점이다. 같은 쪽을 가리키는 두 벡터는 큰 양수, 정반대면 음수, **정확히 직각이면 0**이 된다. 그래서 "두 문서가 얼마나 비슷한가"(22장의 코사인 유사도) 같은 물음도, 파고들어 보면 결국 이 내적 계산 하나로 풀린다.
