...
열과 기침이라는 두 사실에서 출발해 '감기 의심'이 쌓이고, 그 위에 '휴식 권고'가 또 한 층 얹힌다. 데이터에서 출발해 *결론을 향해 쌓아 올리므로* 이것을 **데이터 주도(data-driven)** 방식이라 부른다. 묻는 말은 늘 한결같다 — "지금 아는 것으로 무엇을 더 알 수 있나?" 모니터링이나 실시간 경보처럼 사실이 끊임없이 흘러드는 상황에서, 이 방식은 들어오는 정보마다 곧장 새 결론으로 번역해낸다.
...
전향이든 후향이든, 지금까지의 길은 모두 규칙이라는 톱니바퀴를 처음부터 굴려 결론을 빚어내는 방식이었다. 그런데 사람은 늘 그렇게 추론하지 않는다. 어려운 문제를 만나면 우리는 종종 규칙을 처음부터 따지는 대신, 예전에 풀어 본 **비슷한 사례**를 먼저 떠올린다. 그 과거의 해법을 새 문제에 맞게 조금 고쳐 쓰는 추론, 그것이 **사례 기반 추론(case-based reasoning)**이다. 의사가 눈앞의 환자를 두고 전에 보았던 비슷한 환자를 떠올리고, 변호사가 판례를 뒤지고, 정비사가 같은 증상을 보였던 차를 회상하는 일이 모두 이것이다. 형식으로 옮기면 대개 네 걸음을 밟는다 — 비슷한 사례를 **검색**하고(retrieve), 그 해법을 새 상황에 맞게 고쳐 **재사용**하고(reuse), 실제로 맞는지 **수정**하고(revise), 잘 통한 새 해법을 다시 기억에 **보관**해(retain) 다음을 대비한다. 규칙을 일일이 짜 두기 어려운 분야에서, 쌓인 경험 그 자체를 추론의 밑천으로 삼는 길이다.
